...
from ai_contracts import SafeReviewSet, SafeReview, ResponseStatus
...
def temp_fake_safe_review_set(product_id):
    """TẠM THỜI thay cho A1.2 (guardrails.filter_reviews sau này).

    Hàm này KHÔNG lọc PII, KHÔNG chặn injection — chỉ chuyển review thô
    sang đúng schema SafeReviewSet để A1.1 có thể tích hợp và test được
    ngay, không cần chờ A1.2 xong. Khi A1.2 merge xong, chỉ cần đổi dòng
    gọi hàm này thành guardrails.filter_reviews(...) — không cần sửa gì
    trong grounding.py hay chỗ gọi summarize_with_grounding bên dưới.
    """
    records = fetch_product_reviews_from_db(product_id)
    if not records:
        return SafeReviewSet(
            product_id=product_id,
            reviews=[],
            reason="No reviews found for this product",
        )
    reviews = [
        SafeReview(source_id=f"r{i}", text=row[1], score=row[2])
        for i, row in enumerate(records)
    ]
    return SafeReviewSet(product_id=product_id, reviews=reviews)
...
def get_ai_assistant_response(request_product_id, question):
    ...
    with tracer.start_as_current_span("get_ai_assistant_response") as span:
        ...
        if tool_calls:
            ...
            # Grounding chỉ áp dụng khi có review thật để trích dẫn.
            reviews_were_fetched = any(
                tc.function.name == "fetch_product_reviews" for tc in tool_calls
            )

            if reviews_were_fetched:
                llm_inaccurate_response = check_feature_flag("llmInaccurateResponse")
                logger.info(f"llmInaccurateResponse feature flag: {llm_inaccurate_response}")
                # Flag này được giữ lại có chủ đích: dù model bị ép trả lời
                # sai, grounding.py vẫn phải lọc được claim không có nguồn.
                # Xem test_grounding_filters_fabricated_claim_from_inaccurate_flag
                # trong tests/test_grounding.py.

                # TODO(A1.2): đổi dòng dưới thành guardrails.filter_reviews(...)
                # khi A1.2 merge xong. Không cần sửa gì khác.
                safe_review_set = temp_fake_safe_review_set(request_product_id)

                grounded_result = summarize_with_grounding(safe_review_set, client, llm_model)

                span.set_attribute("app.ai.grounding_status", grounded_result.status.value)
                if grounded_result.status == ResponseStatus.GROUNDED:
                    span.set_attribute("app.ai.claim_count", len(grounded_result.claims))
                    ai_assistant_response.response = grounded_result.answer
                else:
                    ai_assistant_response.response = (
                        grounded_result.reason
                        or "Chua co du thong tin de tra loi cau hoi nay."
                    )

                logger.info(
                    f"Grounded response status={grounded_result.status}, "
                    f"answer='{ai_assistant_response.response}'"
                )
            else:
                # Chỉ gọi fetch_product_info, không có review nào để làm
                # bằng chứng — giữ nguyên luồng tổng hợp trực tiếp như cũ.
                messages.append(
                    {
                        "role": "user",
                ...
                ai_assistant_response.response = final_response.choices[0].message.content
                logger.info(f"Returning a non-grounded AI assistant response: '{ai_assistant_response.response}'")

        else:
            # Model trả lời trực tiếp, không gọi tool nào — không có bằng
            # chứng nào để trích dẫn, nên phải từ chối thay vì trả nội
            # dung model tự sinh ra (đúng invariant 5 trong contract).
            ai_assistant_response.response = "Xin loi, toi can them thong tin tu review de tra loi cau hoi nay."
            logger.info("No tool was called; abstaining since there is no evidence to ground on.")
