...
class RuntimeConfigTest(unittest.TestCase):
    ...
    def test_prometheus_services_expand_generated_metrics(self):
        ...
        self.assertTrue(all("vector(0)" not in query for query_id, query in config.prometheus_queries.items() if not query_id.startswith("llm.")))
        self.assertIn("or on() vector(0)", config.prometheus_queries["llm.model_error_rate_5m"])
        self.assertIn("app_ai_outcome=\"error\"", config.prometheus_queries["llm.model_error_rate_5m"])
        self.assertIn("app_ai_model_tokens_total", config.prometheus_queries["llm.token_usage_5m"])
        self.assertIn("app_ai_model_cost_usd_USD_total", config.prometheus_queries["llm.cost_usd_5m"])
        self.assertIn("traces_span_metrics_duration_milliseconds_bucket", config.prometheus_queries["llm.p95_latency_5m"])
        self.assertIn("app_ai_outcome=\"fallback\"", config.prometheus_queries["llm.fallback_rate_5m"])
