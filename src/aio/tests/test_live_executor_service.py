...
def test_live_executor_http_contract_supports_runtime_self_heal_lifecycle(tmp_path: Path) -> None:
    service, gateway = _live_service(tmp_path / "executor.sqlite3")
    app = create_app(service=service, token="test-token")
    client = TestClient(app)
    ...
    try:
        ...
        plan_request.pop("kubernetes_snapshot")
        plan_response = client.post(
            "/v1/actions/plan",
            json=plan_request,
            headers=headers(plan_request["request_id"]),
        )
        ...
        execute_request = {
            **plan_request,
            "request_id": "req-contract-execute",
            "dry_run": False,
            "plan_hash": plan["plan_hash"],
            "rollback_token": plan["rollback"]["rollback_token"],
            "idempotency_key": "sha256:6060606060606060606060606060606060606060606060606060606060606060",
        }
        execute_response = client.post(
            "/v1/actions/execute",
            json=execute_request,
            headers=headers(execute_request["request_id"]),
        )
        ...
        verification_request = {
            "request_id": "req-contract-verify",
            "incident_id": plan_request["incident_id"],
            "idempotency_key": "sha256:7070707070707070707070707070707070707070707070707070707070707070",
            "passed": False,
            "query_id": "product-catalog.cpu_millicores",
        ...
        verification_response = client.post(
            f"/v1/actions/{execution['execution_id']}/verification",
            json=verification_request,
            headers=headers(verification_request["request_id"]),
        )
        assert verification_response.status_code == 200
        assert verification_response.json()["status"] == "failed"

        rollback_request = {
            "request_id": "req-contract-rollback",
            "incident_id": plan_request["incident_id"],
            "rollback_token": execution["rollback"]["rollback_token"],
            "reason": "verification_failed",
            "requested_by": "aiops-runtime",
            "idempotency_key": "sha256:8080808080808080808080808080808080808080808080808080808080808080",
        }
        rollback_response = client.post(
            f"/v1/actions/{execution['execution_id']}/rollback",
            json=rollback_request,
            headers=headers(rollback_request["request_id"]),
        )
