...
**Hạn nộp:** Thứ Bảy 18/07/2026 **Tác giả: Ngô Nguyên Phúc, Nguyễn Qúy Hưng**
...
| Bằng chứng                         | Link cố định                                                                            | Phạm vi                                                                                               |
| ---------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Detector + baseline implementation | `[d475fd7](https://github.com/tf2-team/tf2-corp-platform/commit/d475fd7)`               | Thêm EWMA, Isolation Forest, BARO/BOCPD, statistical baseline helpers, RCA integration và unit tests. |
| Baseline normalization hardening   | `[6afc18f](https://github.com/tf2-team/tf2-corp-platform/commit/6afc18f)`               | Thay min/max normalization bằng `robust_spread` để baseline ít nhạy với outlier.                      |
...
| #   | Task                                                | Trạng thái                                                                                                                                     |
| --- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Kết nối live Prometheus TF2                         | **DONE** — live snapshots trong evidence #7b.                                                                                                  |
| 2   | Bao phủ checkout p95, cart error-rate và saturation | **PARTIAL** — checkout p95 + cart error đạt; ad/catalog CPU không materialize nên công bố minh bạch checkout memory làm saturation substitute. |
...
*Chặng #7b đã có evidence report, labeled dataset và ADR riêng trong* `docs/mandates/7b/`*.*