...
class SQLiteIncidentStore:
    ...
    def upsert(self, candidate: CandidateEvent) -> Incident:
        ...
        suppressed_by = self._suppression_parent(candidate.service) if is_new or is_reopened else None
        notification = NotificationBuilder().build([incident])[0] if (is_new or is_reopened) and suppressed_by is None else None
