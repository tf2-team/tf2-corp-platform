...
> - This plan is aligned to Phase 3 requirements, `aio-docs/note.md`, the platform split into `tf2-corp-platform`, `aio-docs/architect.md`, `aio-docs/implement_plan.md`, and `aio-docs/aiops/AIO_BACKLOG.md`.
...
| Item | Detail |
|---|---|
| Source-of-truth order | Active BTC mandates; Phase 3 rules, SLO, budget, architecture, and incident history; live TF2 evidence; `architect.md`; `implement_plan.md`; corrected AIOps backlog; Week 1 discovery docs. |
| Repository split | Runtime code, canonical runbooks, production config, tests, and local Grafana assets go under `tf2-corp-platform`. ADRs, evidence, Ops Reviews, postmortems, and evaluation reports go under `aio-docs/aiops`. Runtime must not read from `aio-docs`. |
| Current repo evidence | `tf2-corp-platform/src/aio/` does not exist yet. No `aio-docs/aiops/adr/` directory, canonical AIOps runbooks, runtime tests, EKS deployment proof, or evidence index were found. |
...
| Production realness | Production config cannot contain placeholders, localhost/example URLs, fake adapters, test fixture paths, unverified metrics, floating production tags, or dependencies on `aio-docs`. |
...
|---|---|---|---|
| **Member A** | Detection & Data Engineer | SLI mapping, Prometheus signals, signal qualification, feature computation, detectors, anomaly detection, topology correlation, detection evaluation | Blue |
| **Member B** | Remediation & Ops Engineer | Incident lifecycle, runbooks, dry-run engine, safety guardrails, verification, rollback/escalation behavior, postmortems | Green |
| **Member C** | Observability & Evidence Lead | Grafana rules/dashboards, alert routing, EKS/Helm coordination with CDO, ADRs, evidence index, Ops Reviews, final readout | Yellow |
...
| Dry-run first | Default mode is dry-run. Live action is optional and requires one exact approved action plus all safety, approval, RBAC, cost, verification, and rollback gates. | ADR-SAFETY-001, ADR-LIVE-001 or dry-run-only decision, runtime mode evidence, and guardrail tests. |
| Platform ownership | Real EKS chart/release ownership must be recorded in ADR-DEPLOY-001. The Phase 3 reference chart cannot be treated as writable production source by assumption. | ADR-DEPLOY-001 with chart URL/revision/owner, rendered manifests, Helm release proof, and CDO sign-off. |

---

## WEEK 2 - P0 Detection, Alerting & Dry-Run Response (Jul 13-17)

> **Goal:** Deliver a continuous P0 vertical slice: collect/receive -> qualify -> compute features -> detect -> correlate -> incident -> notify/runbook/evidence -> safety-check -> dry-run -> verify/escalate.

### WEEK 2 - Day 1 (Mon Jul 13)

| Task Name | Assignee | Description | Status | Due Date | Priority | Tags |
|---|---|---|---|---|---|---|
| **Create AIOps service scaffold in `tf2-corp-platform`** | **Member A** | Work: Create `tf2-corp-platform/src/aio/` with README, pyproject, Dockerfile, `.dockerignore`, Python package directories, config directories, scripts, canonical runbook directory, and tests aligned to `architect.md` Section 15 and `implement_plan.md` Section 4. Use one Python modular-monolith service with strict internal module boundaries. Evidence required: directory tree exists, package imports successfully, health app can start locally, dependency lock exists, Dockerfile exists, and no runtime file imports from `aio-docs`. | TO DO | Jul 13 (Mon) | P0 Urgent | `scaffold`, `runtime`, `python`, `tf2-corp-platform` |
| **Implement configuration schemas and domain contracts** | **Member A** | Work: Implement typed schemas/models for settings, signals, detectors, topology, action policies, notification routes, observations, features, incidents, actions, audit events, and runtime mode. Reject duplicate IDs, missing references, placeholder values, hardcoded environment facts, unverified official SLI inputs, unsafe actions, and live mode without approvals. Evidence required: config-check command output, unit tests for invalid configs, canonical config digest output, and sample valid/invalid config fixtures under tests only. | TO DO | Jul 13 (Mon) | P0 Urgent | `config`, `schemas`, `contracts`, `validation` |
...
| **Finalize remediation safety ADR** | **Member B** | Work: Define Detect -> Safety Check -> Dry Run -> optional approved execution -> Verify -> Rollback/Escalate. List allowed recommendations, blocked actions, cooldown, max attempts, one-live-action lock, approval expiry, budget gate, error-budget gate, verification dependency, rollback behavior, and flagd/OpenFeature prohibition. Evidence required: signed `ADR-SAFETY-001`, blocked-action matrix, policy evaluation order, and a dry-run-only or live-action decision link. | TO DO | Jul 13 (Mon) | P0 Urgent | `remediation`, `safety`, `adr` |
| **Finalize alert routing and EKS integration ADR** | **Member C** | Work: Define incident payload, severity, dedup key, real TF2 contact point, direct Grafana route, AIOps webhook route, webhook secret, EKS runtime, chart ownership, namespace, resources, storage, RBAC, and self-metrics ingestion path. Evidence required: signed `ADR-ROUTING-001`, signed `ADR-DEPLOY-001`, redacted alert channel proof, chart URL/revision/owner, and real namespace/release evidence. | GATED | Jul 13 (Mon) | P0 Urgent | `alerting`, `eks`, `adr`, `cdo` |
...
| Task Name | Assignee | Description | Status | Due Date | Priority | Tags |
|---|---|---|---|---|---|---|
| **Implement storage, migrations, and append-only audit** | **Member B** | Work: Implement SQLite WAL storage with forward-only migrations for incidents, incident events, actions, audit events, notification outbox, signal snapshots, config revisions, scheduler checkpoints, and evidence metadata. Pending/executing actions after restart must require human review and never replay automatically. Evidence required: migration test output, restart recovery test, append-only audit test, schema file, and transaction-boundary tests. | TO DO | Jul 14 (Tue) | P0 Urgent | `storage`, `sqlite`, `audit`, `recovery` |
...
| **Build P0 incident runbooks** | **Member B** | Work: Create canonical runbooks under `tf2-corp-platform/src/aio/runbooks/` for official SLO breach, checkout dependency failure, DB saturation, and monitoring loss. Each runbook must include trigger, evidence, first response, prohibited actions, dry-run recommendation, verification, rollback/escalation, owner, and communication template. Evidence required: runbook files, runbook index, runbook matcher test output, and sample incident linking to each runbook. | TO DO | Jul 14 (Tue) | P0 Urgent | `runbook`, `incident-response`, `p0` |
...
| **Implement topology correlation, incident lifecycle, and deduplication** | **Member B** | Work: Implement topology model, temporal grouping, likely-cause scoring, confidence contributions, cascade handling, stable fingerprinting, incident state machine, reopen/recovery, cooldown/suppression, and restart-safe timelines. Default fingerprint includes environment, detector ID, flow, primary service, and likely dependency. Evidence required: tests for correlation, cascade grouping, fingerprint stability, illegal transitions, reopen, recovery, cooldown, and restart recovery. | TO DO | Jul 15 (Wed) | P0 Urgent | `incident`, `deduplication`, `correlation`, `topology` |
...
| **Run controlled P0 end-to-end tests** | **Member B** | Work: Test normal traffic, official SLO breach, checkout dependency, unknown checkout failure, DB pressure, monitoring loss, duplicate cascade, notification failure, runtime restart, prohibited action, verification pass/fail/unavailable, and stale optional dependencies. Verify alert, runbook, dry-run, guardrail, audit, evidence, and escalation without changing BTC flags. Evidence required: JSON/Markdown replay results with scenario revision, code/config revision, command, environment, start/end time, and raw artifact paths. | TO DO | Jul 17 (Fri) | P0 Urgent | `testing`, `remediation`, `repro`, `e2e` |
| **Publish Week 2 Ops Review** | **Member C** | Work: Report official SLO/error-budget state, budget, incidents, P0 coverage, MTTD, false alerts, signal gaps, CDO dependencies, current runtime mode, safety posture, and Week 3 proposal. Evidence required: Ops Review under `aio-docs/aiops/ops-reviews/`, linked dashboards, linked evaluation results, current risk register, and explicit Week 3 scope recommendation. | TO DO | Jul 17 (Fri) | P0 Urgent | `ops-review`, `evidence`, `deliverable` |
...
| **Evaluate the single live-remediation gate** | **Member B** | Work: Enable only the exact CDO-approved stateless action if ADR-LIVE-001, real approval provider, approval expiry/digest, separate executor identity, least-privilege RBAC, dry-run evidence, verification, rollback, cost, cooldown, and error-budget gates pass. Otherwise remain dry-run. Evidence required: ADR-LIVE-001 or dry-run-only decision, RBAC proof, approval evidence, dry-run evidence, verification evidence, rollback plan, and audit example. | GATED | Jul 21 (Tue) | P0 Urgent | `remediation`, `live-gate`, `safety` |
...
| **Complete minimum checkout topology and evidence map** | **Member C** | Work: Finalize checkout dependency/blast-radius map with service, owner, metric query, trace query, log query, runbook, stateful/stateless status, replica evidence, remediation restriction, likely symptom, and escalation path per node. Validate with source, Jaeger traces, and deployed Kubernetes state. Evidence required: topology artifact under `aio-docs/aiops/topology/`, trace validation, K8s validation, and links from config/runbooks. | TO DO | Jul 22 (Wed) | P1 High | `ops-07`, `topology`, `evidence` |
| **Verify Kafka signals and conditionally implement OPS-06** | **Member A** | Work: Discover exact TF2 Kafka metric names and labels. Implement lag/error detector only if verified or promoted by incident/directive. If missing, record N/A and keep detector disabled. Do not assume metric names from collector config alone. Evidence required: Kafka signal qualification evidence, live query output, detector config if enabled, or explicit N/A/defer evidence if disabled. | TO DO | Jul 22 (Wed) | P2 Conditional | `ops-06`, `kafka`, `conditional` |
...
| **Prepare final AIOps evidence pack** | **Member C** | Work: Index SLI mapping, signal inventory, dashboard exports, alert examples, EKS runtime proof, image digest, Helm/chart revision, rendered manifests, RBAC proof, runtime endpoint, alert channel, incidents, ADRs, eval outputs, SLO/budget state, guardrail evidence, gaps, deferred work, and remaining risk. Evidence required: complete `aio-docs/aiops/evidence-index.md` with links to every artifact needed for final readout. | TO DO | Jul 23 (Thu) | P0 Urgent | `readout`, `evidence`, `final` |
...
| **Capture operational evidence** | `ALL` | Work: Save query, trace/log link, dashboard export, command output, chart revision, image digest, runtime endpoint, alert payload, decision, action, verification result, owner, and timestamp as work happens. Evidence required: updated `aio-docs/aiops/evidence-index.md` with artifact link, owner, timestamp, and what acceptance criterion it supports. | RECURRING | Daily | Normal | `evidence`, `auditability` |
| **Check TF2 budget state with CDO** | **Member C** | Work: Record current AWS cost snapshot, weekly headroom, cost anomaly status, and any cost-changing planned action. Unknown or stale cost state blocks live cost-changing action. Evidence required: daily budget entry or CDO cost-feed reference with timestamp, source, current spend, weekly headroom, and owner. | RECURRING | Daily | Normal | `budget`, `cost`, `cdo` |
...
| Continuous multi-signal AIOps runtime on EKS | TO DO | Runtime source, image digest, rendered manifests, pod health, endpoint proof, self-metrics proof, and resource/RBAC evidence. |
| Checkout dependency and DB saturation detection | TO DO | Detector code/config, signal qualification, DB threshold ADR, unit/replay tests, and sample incidents. |
...
| Real TF2 alert routing with deduplication and evidence links | TO DO | Alert route config, webhook auth proof, notification payload contract, dedup tests, and real test alert delivery. |
...
| At most one live action, only if every approval and safety gate passes | GATED | ADR-LIVE-001 with one action and all proofs, or signed dry-run-only decision. |
...
| Signed ADRs for SLI, detection, thresholds, safety, routing, and deployment | TO DO | ADR files under `aio-docs/aiops/adr/` with owners, evidence links, status, decision, and signatures. |
| COE/postmortem for every real incident handled | TO DO | COE template plus completed signed COE for each real incident. |
| Week 2 and Week 3 Ops Reviews | TO DO | Ops Review documents under `aio-docs/aiops/ops-reviews/` with SLO, budget, incidents, coverage, gaps, and next actions. |
...
> - Put production runtime code, canonical runbooks, production config, or fixture-dependent logic under `aio-docs`.
