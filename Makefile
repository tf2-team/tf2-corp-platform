...
remove-multiplatform-builder:
	docker buildx rm techx-corp-builder
...
build-multiplatform-and-push:
    # Because buildx bake does not support --env-file yet, we need to load it into the environment first.
    # Pushes only the release group (22 deployable services, including opensearch); see docker-bake.hcl.
...
generate-kubernetes-manifests:
	...
	helm repo update
	echo "# Copyright The OpenTelemetry Authors" > kubernetes/techx-corp.yaml
	echo "# SPDX-License-Identifier: Apache-2.0" >> kubernetes/techx-corp.yaml
	echo "# This file is generated by 'make generate-kubernetes-manifests'" >> kubernetes/techx-corp.yaml
	echo "---" >> kubernetes/techx-corp.yaml
	echo "apiVersion: v1" >> kubernetes/techx-corp.yaml
	echo "kind: Namespace" >> kubernetes/techx-corp.yaml
	echo "metadata:" >> kubernetes/techx-corp.yaml
	echo "  name: techx-corp" >> kubernetes/techx-corp.yaml
	helm template techx-corp open-telemetry/techx-corp --namespace techx-corp | sed '/helm.sh\/chart\:/d' | sed '/helm.sh\/hook/d' | sed '/managed-by\: Helm/d' >> kubernetes/techx-corp.yaml
...
start:
	...
	@echo ""
	@echo "TechX Corp Platform is running."
...
start-minimal:
	...
	@echo ""
	@echo "TechX Corp Platform in minimal mode is running."
	@echo "Go to http://localhost:8080 for the demo UI."
	@echo "Go to http://localhost:8080/jaeger/ui for the Jaeger UI."
	@echo "Go to http://localhost:8080/grafana/ for the Grafana UI."
	@echo "Go to http://localhost:8080/loadgen/ for the Load Generator UI."
...
stop:
	...
	@echo ""
	@echo "TechX Corp Platform is stopped."
...
# Change trail: @hungxqt - 2026-07-19 - Clear GHA cache flags on local multiplatform ECR push.
